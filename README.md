...
### Schema
The table below describes the reasoning behind character limits for `VARCHAR` and `FLOAT` datatypes for the schema. The goal was to capture the minimum required and allow future entries without causing errors. For some, the character limit is retained for data consistency and integrity.
